...
Masternodes, once unique to the Historia network, are now becoming popular
...
the services they provide to the Historia network specifically.

Simply put, a masternode is a server with a full copy of the Historia
...
transaction features in Historia are called. The masternodes are paid for
...
condition that needs to be fulfilled: proof of ownership of 1000 Historia.
...
implement. This is where the true strength of Historia lies - an
incentivized system of thousands of distributed servers working 24x7
means that Historia can scale more efficiently and deploy services more
quickly than a blockchain run entirely by unpaid volunteers. The more
masternodes, the better and safer the Historia network.

As of November 2018, the Historia network has `over 5000 masternodes located
<http://178.254.23.111/~pub/masternode_count.png>`_ in over `45
countries <https://chainz.cryptoid.info/historia/masternodes.dws>`_ and
hosted on `over 140 ISPs
<http://178.254.23.111/~pub/Historia/masternode_ISPs.html>`_. The block
reward is approximately 3.34 Historia, so the selected masternode receives
1.67 Historia per payment or approximately 6 Historia per month. The block
...
See `this tool <https://historia- news.de/historiatv/#value=1000>`_ to calculate
real-time payment rates, and `this site
<http://178.254.23.111/~pub/Historia/Historia_Info.html>`_ for various real-time
...
- It is possible to upgrade an existing masternode in-place without 
  starting a new server and without moving your 1000 HISTORIA collateral.
- A masternode was previously "started" using the ``masternode start-alias`` 
  ...
  "started". Masternodes begin offering services when a `ProRegTx <https://github.com/HistoriaOffical/dips/blob/master/dip-0003.md#registering-a-masternode-proregtx>`_ 
  `special transaction <https://github.com/HistoriaOffical/dips/blob/master/dip-0002.md>`_ 
...
- The ProRegTx contains 2 Historia addresses (also called public keys) and
  one BLS public key, which represent 3 different roles in the
  masternode and define update and voting rights. The keys are:
  
  1. ``ownerKeyAddr``: This is a Historia address (public key) controlled by
  ...
  3. ``votingKeyAddr``: This is a Historia address (public key) used for
...
- The masternode configuration can later be updated using ProUpServTx,
  ...
  Information <https://github.com/HistoriaOffical/dips/blob/master/dip-0003.md#updating-masternode-information>`_ 
...
- All functions related to DIP003 will only take effect once Spork 15 is
  ...
  masternode following the `old process <https://docs.historia.network/en/0.12.3/masternodes/setup.html>`_ 
...
2. Install the Historia software and synchronize the blockchain
...
a Historia Wallet (or DMT). Step 5 requires access to the wallet actually
holding the collateral. Step 6 requires a Historia balance to pay the
transaction fee.
...
Historia and Bitcoin, the consensus mechanism is Proof of Work (PoW).
...
Masternode payments in Historia version 0.13.0 are entirely deterministic
...
<https://docs.historia.network/en/0.12.3/masternodes/understanding.html#payment-logic>`_. 
Historia version 0.13.0 implements `DIP003
<https://github.com/HistoriaOffical/dips/blob/master/dip-0003.md>`_ and defines
...
of Historia 0.14 are:

- Failure to participate in `DKG <https://github.com/HistoriaOffical/dips/blob/master/dip-0006.md#llmq-dkg-network-protocol>`__\ = 66% punishment
- Each subsequent block reduces PoSe score by 1

Quorum selection
================

InstantSend transactions in Historia version 0.13.0 are secured using a
...
- 1000 Historia: Arguably the hardest part. Historia can be obtained from
...
In addition to the 1000 Historia held in collateral, masternodes also have
...
The exact hardware requirements for Historia Evolution masternodes have yet
to be determined, although some pointers can be taken from the `roadmap
<https://github.com/HistoriaOffical/historia-roadmap>`_ and this `blog post
<https://medium.com/@eduffield222/how-to-enabling-on-chain-scaling-2ffab5997f8b>`_. 
It should be possible to run Historia masternodes on normal VPS servers
