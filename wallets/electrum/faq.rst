.. meta::
   :description: Frequently Asked Questions about the Historia Electrum wallet
   :keywords: historia, wallet, electrum, faq, seed
...
How does Historia Electrum work?
----------------------------

Historia Electrum focuses on speed, low resource usage and providing a
simple user experience for Historia. Startup times are instant because it
operates in conjunction with high-performance servers that handle the
most complicated parts of the Historia system.

Does Historia Electrum trust servers?
---------------------------------

Not really; the Historia Electrum client never sends private keys to the
...
The seed created by Historia Electrum has 128 bits of entropy. This means
that it provides the same level of security as a Historia private key (of
...
The Historia Electrum wallet design and workflow are based on a concept
called a “wallet generation seed”. This seed is a unique, randomly-
selected list of twelve words. A Historia Electrum wallet uses its seed as a
...
.. figure:: img/change-addresses.png
   ...
   Receiving and change addresses in Historia Electrum
...
Historia is an electronic cash system, meaning that it shares much in common
...
surprisingly, this is how Historia transactions work as well. If the entire
...
How can I send Historia without paying a transaction fee?
-----------------------------------------------------
...
Note that transactions without fees might not be relayed by the Historia
Electrum server, or by the Historia network.

Is there a way to enter amounts in USD in Historia Electrum?
--------------------------------------------------------
...
.. figure:: img/faq-fiat.png
   ...
   Entering values in fiat currency in Historia Electrum


What does it mean to “Freeze” an address in Historia Electrum?
----------------------------------------------------------

When you freeze an address, the funds in that address will not be used
for sending Historia. You cannot send Historia if you don’t have enough funds
in your non-frozen addresses.

How is the wallet encrypted?
----------------------------

Historia Electrum uses two separate levels of encryption:
 ...
 - In addition, your wallet file may be encrypted on disk. Note that the
   ...
   password will not be kept in memory; Historia Electrum does not need it
...
requesting Historia on a wallet that you do not control.
...
Does Historia Electrum support cold wallets?
----------------------------------------

Yes. See the :ref:`cold storage <electrum-cold-storage>` section.

Can I import private keys from other Historia clients?
--------------------------------------------------

In Historia Electrum 2.0, you cannot import private keys in a wallet that
...
Can I sweep private keys from other Historia clients?
-------------------------------------------------

Sweeping private keys means to send all the Historia they control to an
existing address in your wallet. The private keys you sweep do not
become a part of your wallet. Instead, all the Historia they control are
...
unchanged. This is the destination address from your existing Historia
Electrum wallet. Click on **Sweep**. Historia Electrum then takes you to the
...
Can I do bulk payments with Historia Electrum?
------------------------------------------
...
.. figure:: img/faq-paytomany.png
   ...
   Creating a transaction with multiple outputs in Historia Electrum
...
Can Historia Electrum create and sign raw transactions?
---------------------------------------------------

Historia Electrum lets you create and sign raw transactions right from the
user interface using a form.

Historia Electrum freezes when I try to send Historia
---------------------------------------------
...
donations from a Historia faucet). When you send Historia, Historia Electrum looks
...
sending smaller amounts of Historia to one of your wallet addresses; this
...
your deterministic sequence of addresses. Historia Electrum uses it in order
to stop looking for addresses. In Historia Electrum 2.0, it is set to 20 by
...
Historia Electrum will generate new addresses as you use them, until it hits
the `gap limit`_.
...
How to upgrade Historia Electrum?
-----------------------------

Warning: always save your wallet seed on paper before doing an upgrade.

To upgrade Historia Electrum, just :ref:`install <electrum-installation>`
...
Some Historia Electrum upgrades will modify the format of your wallet files.
For this reason, it is not recommended to downgrade Historia Electrum to an
...
The following issues should be considered when upgrading Historia Electrum
1.x wallets to Historia Electrum 2.x:

- Historia Electrum 2.x will need to regenerate all of your addresses during
  the upgrade process. Please allow it time to complete, and expect it
  to take a little longer than usual for Historia Electrum to be ready.

- The contents of your wallet file will be replaced with a Historia
  Electrum 2 wallet. This means Historia Electrum 1.x will no longer be able
  to use your wallet once the upgrade is complete.

- The **Addresses** tab will not show any addresses the first time you
  launch Historia Electrum 2. This is expected behaviour. Restart Historia
  Electrum 2 after the upgrade is complete and your addresses will be
  available.

- Offline copies of Historia Electrum will not show the addresses at all
  ...
  Console: `wallet.synchronize()`. When it’s complete, restart Historia
